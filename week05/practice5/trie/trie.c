...
bool check(char *word)
{
    // Set pointer, cursor, to root
    node *cursor = root;

    // Iterate through every letter of word
    for (int k = 0; k < strlen(word); k++)
    {
        // Define current indx as the current letter's alphabetical position
        int indx = tolower(word[k]) - 'a';
        // Check existance of current letter in node's childrens
        if (cursor->children[indx] != NULL)
        {
            // Move cursor to children node
            cursor = cursor->children[indx];
        }
        else
        {
            return false;
    ...
    // After checking all letters, confirm that end of the tree is indeed a word
    if (cursor->is_word)
    {
        return true;
    }
